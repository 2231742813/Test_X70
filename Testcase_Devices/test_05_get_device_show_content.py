...
#  取可变信息标志的当前显示内容
class Test_05_get_device_show_content:
    @allure.epic('X70')
    @allure.feature('设备模块')
    @allure.story('基本功能')
    @allure.title('用例标题：A005-取可变信息标志的当前显示内容')
    @allure.severity('blocker')
    def test_05_get_device_show_content(self):
        ...
        msg = msg[8:length-13:]
        print("\n---------------------------------------------------------------------------")
        print("取可变信息标志的当前显示内容")
        print("返回信息 {}".format(msg))
        print("序号 {}".format(msg[0:3:]))
        print("停留时间 {}".format(msg[3:8:]))
        print("出字方式 {}".format(msg[8:10:]))
        print("出字速度 {}".format(msg[10:15:]))
        print("显示字符串 {}".format(msg[15:len(msg):]))
        print("---------------------------------------------------------------------------")
